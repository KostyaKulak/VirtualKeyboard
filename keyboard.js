...
class Keyboard {
  ...
  init () {
    ...
    this.buttons.forEach(button =>
      button.render(this.locale, false, this.buttons.indexOf(button), this.CapsLock)
    )
  ...
  render () {
    ...
    this.buttons.forEach(button =>
      button.render(this.locale, this.caps, this.buttons.indexOf(button), this.CapsLock, this.Shift)
    )
    this.showInfo()
  }
  ...
  keyHandle (e) {
    ...
    if (e.type === 'keydown' && (e.code === 'ArrowUp' || e.code === 'ArrowDown')) {
  ...
  clickHandle (e) {
    ...
    if (el.dataset && el.dataset.buttonPos) {
      ...
      if (el.dataset.buttonType === 'ShiftLeft' || el.dataset.buttonType === 'ShiftRight') {
        this.caps = !this.caps
        this.Shift = !this.Shift
        this.render()
      }

      if (el.dataset.buttonType === 'ControlLeft' && this.Shift && !this.CapsLock) {
        this.locale = (this.locale === 'en') ? 'ru' : 'en'
        localStorage.locale = this.locale
        this.render()
      }

      if (el.dataset.buttonType === 'ArrowLeft' || el.dataset.buttonType === 'ArrowRight') {
        if (el.dataset.buttonType === 'ArrowLeft') this.changeSelectionStart(-1)
        if (el.dataset.buttonType === 'ArrowRight') this.changeSelectionStart(1)
      }

      if (el.dataset.buttonType === 'ArrowUp' || el.dataset.buttonType === 'ArrowDown') {
  ...
  printer (char) {
    this.editor.setRangeText(char, this.editor.selectionStart, this.editor.selectionEnd, 'end')
    this.editor.focus()
  }
  ...
  changeSelectionStart (num) {
    ...
    if (num > 0) {
      pos = (this.editor.selectionStart === this.getLength) ? this.getLength : pos
    }
  ...
  deleteChar (num) {
    ...
    if (startPos !== endPos) {
      this.editor.value = str.slice(0, startPos) + str.slice(endPos, str.length)
    } else {
